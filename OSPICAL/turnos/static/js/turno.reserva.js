...
ListaDesplegable.prototype = {
    ...
    /**
     * objeto al cual se le notificaran los cambios de estado
     */
    mediador : [],
    ...
    /**
     * Notifica el cambio de estado utilizando las funciones de callback
     */
    notificar : function() {
        this.mediador.actualizar(this);
    },
     ...
     * 
     * @param url:
     *            (opcional) url donde se buscara la lista de objetos que
     *            rellenaran la lista
     * @param error_mensaje:
     *            (opcional) mensaje de error en caso que la lista de elementos
     *            este vacia
     * @param error_titulo:
     *            (opcional) titulo del mensaje de error
     */
    ...
    seleccionar : function(index) {
        ...
        if (index >= 0 && index < this.elementos.length) {
            ...
            // (+1 por elemento por defecto 'Seleccionar')
            this.dom.prop('selectedIndex', index + 1);
...
Dias.prototype.cargar = function(especialista) {
    ...
    $.each(this.elementos, function(index, value) {
        ...
        options += '<option value="{0}">{4}{1}, {2} de {3}</option>'.format(
                milis, DIA[fecha.getDay()], fecha.getDate(), MES[fecha
                        .getMonth()], estado);
 ...
 * Obtiene los elementos que compondran la lista desplegable.
 */
TurnosDisponibles.prototype.obtenerElementos = function(milis) {
    if (this.especialista && milis) {
        // configuro los parametros para buscar los elementos de la lista
        var d = new Date(parseInt(milis));
        var url = '/json/turnos/{0}/{1}/{2}/{3}/'.format(this.especialista,
                                                         d.getFullYear(),
                                                         d.getMonth() + 1, 
                                                         d.getDate());
...
Mediador = function() {
    ...
    // FIXME: Ver si esto se puede hacer de mejor manera
...
Mediador.prototype = {
    /**
     * Array que asocia los "colegas" con el metodo que se llamara cada vez
    ...
    init : function() {
        ...
        // configuro el ajax para que funcione en modo asincronico, es decir
        // que espere la respuesta del servidor antes de seguir ejecutando 
        // codigo
        $.ajaxSetup({
            async : false,
        });
    ...
    actualizar : function(obj) {
        ...
        // ejecuto el metodo con el contexto del objeto mediador y paso como
        // parametro al objeto que se modifico
        if (metodo)
...
$(document).ready(function() {
    ...
    $(':reset').click(function() {
        ...
        $("#id_especialista, #id_dia, #id_hora").prop({
            disabled : true
        });
    });
